...
base_dir = microburst_dispersion.config['here'].parent

interesting_microbust_list = pd.read_excel(
    base_dir / 'data' / 'firebird_interesting_dispersion_examples.xlsx',
    skiprows=1, 
    usecols='A:C'  # We don't need any of the other columns for now.
    )

input_plot_dir = base_dir / 'plots' / 'dispersion_summary'
